...
// ---- who a vaccine is offered to -------------------------------------------
// One place, because the dropdown on the record form and the rows on the card
// must never disagree about this. Two copies of the rule in two templates is
// how you end up with a vaccine you cannot see but can still pick.
//
// A vaccine with no `sex` is for everybody. A patient whose sex was never
// recorded is also offered everything: `sex` is nullable on purpose, and
// refusing HPV to a girl because a field was left blank would be a worse error
// than the one being prevented.
function offeredTo(vaccine, patientSex) {
  if (!vaccine.sex) return true;
  if (!patientSex) return true;
  return String(patientSex).toLowerCase() === vaccine.sex;
}

// Same rule for a built card row, with one addition: a row that already holds
// a dose — given or booked — is always shown. Restricting what can be offered
// is correcting the future; hiding what was already recorded is erasing the
// past, and the card is a medical record before it is a form.
function rowVisible(row, patientSex) {
  if (offeredTo(row, patientSex)) return true;
  return (row.doseSlots || []).some((s) => s.given || s.bookedFor);
}

module.exports = {
  buildCard, overdueCounts, overdueChildrenSql, DUE_ARRAYS,
  dueDateFor, daysBetween, withinEpiAge, offeredTo, rowVisible,
