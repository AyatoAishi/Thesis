...
function dispenseStatusLabel(d) {
  if (!d.requires_doctor_approval) return "Completed";
  return d.approved_at ? "Approved" : "Pending approval";
}
...
function buildPatientRecordPdf(doc, { patient, appointments, dispenses, generatedBy }) {
  ...
  simpleTable(doc, {
    headers: ["Date", "Medicine", "Qty", "Status"],
    rows: dispenses.map((d) => [
      fmtDateTime(d.dispensed_at),
      d.medicine_name,
      `${d.quantity} ${d.unit || ""}`.trim(),
      dispenseStatusLabel(d),
    ]),
    widths: [150, 195, 60, 65],
  });
}

module.exports = { buildPatientRecordPdf };
