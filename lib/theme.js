...
// 2026-09-25: the clinic green. The professors asked for "default color green"
// and for the change-theme option to go, so there is now one look for every
// account. The hue is taken from the leaves on the Barangay Sampaguita seal;
// the seal itself prints them at #003800, which is nearly black on a screen, so
// this is the same hue lifted until it reads as green and still carries white
// text at better than 5:1.
...
// Every account gets the same appearance now. The saved preferences column is
// deliberately left alone — nobody's stored choices are deleted, they are
// simply no longer read — so the setting could be brought back without asking
// anybody to choose again. `rawPrefs` is accepted and ignored for the same
// reason: every caller keeps working unchanged.
const CLINIC_THEME = Object.freeze({ mode: "light", preset: "sampaguita", accent: DEFAULT_ACCENT, font: "jakarta", animations: true });

function forUser(/* rawPrefs — intentionally unused, see above */) {
  const p = normalize(CLINIC_THEME);
...
module.exports = {
  PRESETS, FONTS, MODES, DEFAULT_ACCENT, CLINIC_THEME, allFontsHref,
